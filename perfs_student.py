...
# used to test the work queue implementation with varying chunk sizes
def graphWork(mode, filter = "3x3"):
  local_results = defaultdict(list)
  chunks = [1,2,4,8,16,32]
  for size in chunks:
    for nthread in threads:
      chunk_size = size
      key = (filter, "work queue", nthread, chunk_size, default_file) # 5 for work queue inside that method
      run_perf(*key)
      local_results[size] += [float(results[key][mode])] # mode should be time

  title = ('4M pixels square image, filter = {}, chunk_size (work_queue) = 1-32. Average over 10 runs.'.format(filter))
  ylabel = mode
  if mode == 'time':
    ylabel += "(s)"
  ...
  plotter.graph(
    threads, # x axis
    [local_results[size] for size in chunks], # y vals
    chunks,  # names for each line
    [colours[size] for size in chunks],
    'graph_{}.png'.format(mode+filter+"Chunk"), # filename
    title,
    "# Threads", # xlabel
    ylabel
  )

# used to test the different filters at 8 threads
def graphFilter(mode):
  local_results = defaultdict(list)
  for method in methods:
    for size in filters:
      chunk_size = 8
      key = (size, method, 8, chunk_size, default_file)
      run_perf(*key)
      local_results[method] += [float(results[key][mode])]

  title = ('4M pixels square image, filter = 1 - 4, chunk_size (workqueue) = #threads. Average over 10 runs.')
  ylabel = mode
  if mode == 'time':
    ylabel += "(s)"
  ...
  plotter.graph(
    [filters[n] for n in filters], # x axis
    [local_results[method] for method in methods_as_list], # y vals
    methods_as_list,  # names for each line
    [colours[method] for method in methods_as_list],
    'graph_{}.png'.format(mode+"Varying"), # filename
    title,
    "# filter id", # xlabel
    ylabel
  )
...
# used to test the second image that is 1 pixel wide
def graph2(mode, filter = "3x3"):
  local_results = defaultdict(list)
  for method in methods:
    for nthread in threads:
      chunk_size = nthread
      key = (filter, method, nthread, chunk_size, '2') # instead of default_file, we have 2 (which is the skinny pic)
      run_perf(*key)
      local_results[method] += [float(results[key][mode])]

  title = ('long image, filter = {}, chunk_size (workqueue) = #'
      'threads. Average over 10 runs.'.format(filter))
  ylabel = mode
  if mode == 'time':
    ylabel += "(s)"
  ...
  plotter.graph(
    ...
    methods_as_list,  # names for each line
    [colours[method] for method in methods_as_list],
    'graph_{}_tall.png'.format(mode+filter), # filename
    title,
    "# Threads", # xlabel
    ylabel
  )
...
# additional tests
graphWork('time')
graphWork('l1d_loadmisses')
graphFilter('time')
graphFilter('l1d_loadmisses')
graph2('time')
